...
class ArmInterface:
    ...
    def get_state(self):
        try:
            ...
            ) from None  # suppress original exception

        # also check if gravity compensation has been set by the controller
        if self.gravity_compensation_internal_event.is_set():
            print(
                "Emergency stop (gravity compensation) activated by controller, will not take any more commands"
            )
    ...
    def switch_to_task_compliant_mode(self):
        ...
        try:
            self.arm.switch_to_task_compliant_mode(
                self.command_queue,
                self.gravity_compensation_external_event,
                self.gravity_compensation_internal_event,
            )
        except Exception as e:
            print(f"Error in switch_to_task_compliant_mode: {e}")
            # Re-raise a simplified exception to avoid pickling issues
            raise Exception(
                f"Error in switch_to_task_compliant_mode: {str(e)}"
            ) from None  # suppress original exception
    ...
    def switch_to_joint_compliant_mode(self):
        ...
        try:
            self.arm.switch_to_joint_compliant_mode(
                self.command_queue,
                self.gravity_compensation_external_event,
                self.gravity_compensation_internal_event,
            )
            ...
            # Re-raise a simplified exception to avoid pickling issues
            raise Exception(
                f"Error in switch_to_joint_compliant_mode: {str(e)}"
            ) from None
        self.in_compliant_mode = True
    ...
    def switch_out_of_compliant_mode(self):
        ...
        with self.gravity_compensation_external_event_lock:
            ...
            try:
                ...
                # Re-raise a simplified exception to avoid pickling issues
                raise Exception(
                    f"Error in switch_out_of_compliant_mode: {str(e)}"
                ) from None  # suppress original exception
            self.in_compliant_mode = False
    ...
    def set_joint_position(self, command_pos):
        ...
        try:
            ...
            # Re-raise a simplified exception to avoid pickling issues
            raise Exception(
                f"Error in set_joint_position: {str(e)}"
            ) from None  # suppress original exception
    ...
    def set_joint_trajectory(self, trajectory_command):
        ...
        try:
            ...
            # Re-raise a simplified exception to avoid pickling issues
            raise Exception(
                f"Error in set_joint_trajectory: {str(e)}"
            ) from None  # suppress original exception
    ...
    def close_gripper(self):
        ...
        try:
            ...
            # Re-raise a simplified exception to avoid pickling issues
            raise Exception(
                f"Error in close_gripper: {str(e)}"
            ) from None  # suppress original exception
        ...
        with self.gravity_compensation_external_event_lock:
            ...
            if self.in_compliant_mode:
                ...
                try:
                    ...
                    # Re-raise a simplified exception to avoid pickling issues
                    raise Exception(
                        f"Error in emergency_stop: {str(e)}"
                    ) from None  # suppress original exception
